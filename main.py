...
def main():
    ...
    pattern = re.compile(r"((.lose|.ix|.esolve)(\S*|\s*))(.|)#\d+", re.DOTALL) # remove re.DOTALL
    matches = re.findall(pattern, pull_request_body, re.VERBOSE) # try re.VERBOSE
    matches2 = re.finditer(pattern, pull_request_body, re.VERBOSE)
